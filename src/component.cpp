...
constexpr WarningText kWarnings[] = {
    {"chatguard.message_flood", "Не флудите: подождите пару секунд."},
    {"chatguard.message_repeat", "Не повторяйте одно и то же сообщение."},
    {"chatguard.message_ads", "Реклама в чате запрещена."},
    {"chatguard.message_caps", "Не пишите капсом."},
    {"chatguard.message_bad_word", "Следите за языком."},
};
...
class ChatGuardComponent final : public IComponent,
...
public:
    ...
    static ChatGuardComponent* instance() { return instance_; }
    chatguard::Guard& guard() { return guard_; }
    const chatguard::Guard& guard() const { return guard_; }

    ChatGuardComponent() { instance_ = this; }
    ...
    StringView componentName() const override { return "ChatGuard"; }
    ...
    void provideConfiguration(ILogger&, IEarlyConfig& config, bool defaults) override {
        ...
        const chatguard::Settings d;
        set_bool("chatguard.anti_ads", d.anti_ads);
        set_empty_list("chatguard.allowed_hosts");
        set_bool("chatguard.anti_caps", d.anti_caps);
        set_int("chatguard.caps_min_letters", d.caps_min_letters);
        set_int("chatguard.caps_percent", d.caps_percent);
        set_bool("chatguard.anti_flood", d.anti_flood);
        set_int("chatguard.flood_messages", d.flood_messages);
        set_int("chatguard.flood_interval_ms", static_cast<int>(d.flood_interval.count()));
        set_int("chatguard.repeat_interval_ms", static_cast<int>(d.repeat_interval.count()));
        set_empty_list("chatguard.bad_words");
        set_bool("chatguard.log_blocked", true);
    ...
    void onInit(IComponentList* components) override {
        ...
        if (!pawn_) {
            core_->logLn(LogLevel::Error, "[ChatGuard] Pawn component not found, natives are unavailable");
            return;
        }
    ...
    bool onPlayerText(IPlayer& player, StringView message) override {
        const Reason reason = guard_.check_message(player.getID(), to_std(message), chatguard::Clock::now());
        if (reason == Reason::None) return true;

        if (log_blocked_) {
            core_->logLnU8(LogLevel::Message, "[ChatGuard] %s (%d), %s: %s",
...
private:
    void load_settings(IConfig& config) {
        chatguard::Settings settings;
        settings.anti_ads = read_bool(config, "chatguard.anti_ads");
        for (const auto& host : read_strings(config, "chatguard.allowed_hosts")) {
            settings.allowed_hosts.push_back(cp1251::to_lower(cp1251::from_utf8(host)));
        }
        settings.anti_caps = read_bool(config, "chatguard.anti_caps");
        settings.caps_min_letters = read_int(config, "chatguard.caps_min_letters");
        settings.caps_percent = read_int(config, "chatguard.caps_percent");
        settings.anti_flood = read_bool(config, "chatguard.anti_flood");
        settings.flood_messages = read_int(config, "chatguard.flood_messages");
        settings.flood_interval = std::chrono::milliseconds(read_int(config, "chatguard.flood_interval_ms"));
        settings.repeat_interval = std::chrono::milliseconds(read_int(config, "chatguard.repeat_interval_ms"));
        for (const auto& word : read_strings(config, "chatguard.bad_words")) {
            settings.bad_words.push_back(cp1251::to_lower(cp1251::from_utf8(word)));
        }
        guard_.configure(std::move(settings));

        log_blocked_ = read_bool(config, "chatguard.log_blocked");
        warnings_.clear();
        for (const WarningText& warning : kWarnings) {
    ...
    bool notify_scripts(int playerid, Reason reason, StringView text) {
        ...
        const auto call = [&](IPawnScript* script) {
            if (script && script->Call("OnChatGuardBlock", DefaultReturnValue_True, playerid, static_cast<int>(reason),
                                       text) == 0) {
                show_warning = false;
            }
        };
    ...
    static inline ChatGuardComponent* instance_ = nullptr;

    ICore* core_ = nullptr;
    IPawnComponent* pawn_ = nullptr;
    chatguard::Guard guard_;
    std::vector<std::string> warnings_;
    bool log_blocked_ = true;
};

SCRIPT_API(ChatGuard_CheckText, int(std::string const& text)) {
    const auto* component = ChatGuardComponent::instance();
    return component ? static_cast<int>(chatguard::check_text(text, component->guard().settings())) : 0;
}

SCRIPT_API(ChatGuard_SetImmune, bool(IPlayer& player, bool immune)) {
    auto* component = ChatGuardComponent::instance();
    if (!component) return false;
    component->guard().set_immune(player.getID(), immune);
    return true;
}

SCRIPT_API(ChatGuard_IsImmune, bool(IPlayer& player)) {
    const auto* component = ChatGuardComponent::instance();
    return component && component->guard().is_immune(player.getID());
}

COMPONENT_ENTRY_POINT() {
    return new ChatGuardComponent();
}
