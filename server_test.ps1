# Runs Moderator inside a real open.mp server with testmode\moderator_test.pwn and checks its verdict.
$ErrorActionPreference = 'Stop'

$root = $PSScriptRoot
$dist = Join-Path $root 'deps\server-dist\Server'
$server = Join-Path $root 'build\server'
$dll = Join-Path $root 'build\Moderator.dll'
...
$config.max_bots = 5
$config.pawn.main_scripts = @('moderator_test 1')
$moderator = Get-Content (Join-Path $root 'testmode\moderator.json') -Raw -Encoding UTF8 | ConvertFrom-Json
$config | Add-Member -Force -NotePropertyName moderator -NotePropertyValue $moderator
...
$source = [IO.File]::ReadAllText((Join-Path $root 'testmode\moderator_test.pwn'), [Text.Encoding]::UTF8)
$pwn = Join-Path $server 'gamemodes\moderator_test.pwn'
[IO.File]::WriteAllText($pwn, $source, [Text.Encoding]::GetEncoding(1251))

$qawno = Join-Path $server 'qawno'
& (Join-Path $qawno 'pawncc.exe') $pwn "-i$qawno\include" "-i$root\pawn" "-o$server\gamemodes\moderator_test.amx"
...
$log -split "`r?`n" | Where-Object { $_ -match 'Moderator|moderator-test|error|warning' }
if ($log -notmatch '\[moderator-test\] done: \d+ checks, 0 failed') {
